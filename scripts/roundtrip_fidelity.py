...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for skill_md in skills:
            slug = skill_md.parent.name
            problems, stats = check_skill(skill_md, tk, workdir)
            total_params += stats["params"]
            total_steps += stats["steps"]
            if problems:
                failures.append(slug)
                print(f"FAIL  {slug}  [{stats['capability_id']}]")
    ...
    print(
        f"loop:      raw -> skill -> agent -> skill -> agent  "
        f"({len(skills)} skills x 2 laps)"
    )
    ...
    if failures:
        ...
        return 1
    print(f"result:    {len(skills)}/{len(skills)} preserved capability, no drift")
    return 0
